...
shipments <- read_csv(sprintf('https://api.russiafossiltracker.com/v0/voyage?pricing_scenario=default&format=csv&date_from=2021-12-01&arrival_date_to=2022-10-31&select=%s', shipment_fields))
overland <- read_csv('https://api.russiafossiltracker.com/v0/overland?keep_zeros=False&format=csv&date_from=2021-12-01&date_to=2022-10-31')
...
(counter_manual %>%
    ...
    tidyr::spread(pricing_scenario, value_bneur) %>%
    mutate(diff2=default-pricecap_andrei_2))
...
counter_manual %>%
  ...
  mutate(unit='EUR') %>%
  write_csv('reports/202210_monthly_update/price_cap_ts.csv')
