build_price_models <- function(){
  ...
  trade %>% group_by(commodity, transport) %>%
    group_map(function(df, group) {
      start_year = ifelse(group$commodity=='natural_gas' & group$transport == 'pipeline', 2015, 2015)
  ...
  trade_with_predictions %>% select(-c(model)) %>%
    ...
    ggplot(aes(date, value, col=name)) + facet_grid(transport~commodity) + geom_line()
