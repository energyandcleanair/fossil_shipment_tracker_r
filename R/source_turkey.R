turkey.get_flows <- function() {
  ...
  exim <- eurostat_codes %>% lapply(function(code) eurostat::get_eurostat(id = code, time_format = "date"))
  ...
  exim_combined <- bind_rows(exim, .id = "code")

  flows <- exim_combined %>%
    filter(geo == "TR") %>%
    filter(partner == "RU") %>%
    filter(unit == "MIO_M3") %>%
    filter(lubridate::year(TIME_PERIOD) >= 2018) %>%
    filter(values != 0) %>%
