...
data <- reactive({
  ...
  # Add label
  data <- data %>%
    mutate(label=ifelse(colour==data$colour[[1]],
                        sprintf("%s\n%s: %s %s", strftime(date, '%Y-%m-%d'), colour, round(value/value_scale), unit_label),
                        sprintf("%s: %s %s", colour, round(value/value_scale), unit_label)
                        )) %>%
