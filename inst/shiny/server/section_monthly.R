...
counter_monthly <- reactive({
  ...
  commodity_name <- list('crude_oil'='Crude oil',
  ...
  counter <- counter_raw %>%
    filter(commodity %in% names(commodity_name)) %>%
    mutate(commodity=factor(recode(commodity, !!!commodity_name), levels=rev(names(commodity_colors))))
...
output$plot_monthly <- renderPlotly({
  ...
  commodity_colors <- list('Crude oil'='#35416C',
                           'Oil products'='#990000',
                           'Coal'='#333333',
                           'LNG'='#f6b26b',
                           'Pipeline gas'='#741b47')
  ...
  plt <- ggplot(global_month, aes(month, value_eur / 1e6, fill=commodity)) +
    ...
    geom_text(data=global_month_on_month,
              ...
              vjust=-0.5) +
    rcrea::theme_crea() +
    labs(caption=sprintf("Source: CREA analysis. Updated on %s.", max(counter$date)),
         y="mn EUR / day",
         x=NULL) +
