         ...
         sidebarLayout(
           ...
           mainPanel(
             ...
             div(class="container_trade",
                 h1("European fossil fuel imports from Russia"),
                 div(class="subtitle", "Pipeline and shipments"),
                 plotlyOutput("plot_flows", height='calc(100vh - 180px)') %>% withSpinner(color="#8cc9D0"),
                 div(class="row",
                     div(class="col-xs-12 caption", span("Source: CREA analysis based on ENTSOG, MarineTraffic and AIS.")
                     )
                 )
             )
           )
